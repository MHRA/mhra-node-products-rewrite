...
export class Pagination {
    ...
    /**
     * The cursor pointing to the end of the current page.
     */
    end_cursor: string;
     ...
     */
    static build(offset: number, resultCount: number, totalCount: number) {
        const hasPreviousPage = offset !== 0;
        const hasNextPage = offset + resultCount < totalCount;
        const startCursor = base64.encode(offset.toString());
        const endCursor = base64.encode(
            Math.min(totalCount, offset + resultCount - 1).toString()
        );
     ...
     */
    static convert_after_offset(encoded) : number {
     ...
     */
    static get_offset_or_default(skip, after, defaultValue) : number {
        if (after) {
            try {
