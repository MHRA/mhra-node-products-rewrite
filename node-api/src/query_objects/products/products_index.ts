...
import {
  AzurePagination,
  CollectionCountResults,
  FacetResults,
  GetProductsIndexResponse,
  IndexResults,
  ProductResult,
  SubstanceIndexItem,
} from "../../types.js";
...
export class ProductsDataSource {
  ...
  constructor() {
    this.Context = new AzureContext(
      process.env.AZURE_SEARCH_INDEX,
      process.env.AZURE_SEARCH_BMGF_INDEX
    );
  }
 ...
 */
async get_products_associated_with_substances(substance: string): Promise<SubstanceIndexItem[]> {
    ...
    const product_index = await this.Context.products_client.build_facet_search<
        FacetResults<Product>
    >("facets", facet_match, "eq");
 ...
 */
async filter_products_by_name(product_name: string): Promise<Product> {
    // Build a full search query with filter for product name
    const product_results =
        await this.Context.products_client.build_full_search_with_filter<
            CollectionCountResults<ProductResult>
        >("product_name", product_name, "eq");
    ...
    product_results.value.map((product) => {
        ...
        node.docType = product.doc_type
            ?.toString()
            .toUpperCase() as unknown as DocumentType;
        ...
        node.territoryType = product.territory
            ?.toString()
            .toUpperCase() as unknown as TerritoryType;
 ...
 */
async search_product_documents(
    search: string,
    first: number,
    skip: number,
    after: String,
    document_types: Array<DocumentType>,
    territory_types: Array<TerritoryType>
): Promise<Documents> {
    ...
    const cleaned_fuzzy_search = this.Context.products_client.add_fuzzy_search(
        cleaned_search,
        this.Context.products_client.config.search_fuzziness,
        this.Context.products_client.config.search_exactness_boost
    );
    ...
    let search_data = await this.Context.products_client.build_search<
        CollectionCountResults<ProductResult>
    >(query_string);
    ...
    if (document_types.length >= 1) {
        search_data.value = search_data.value.filter(x =>
            document_types.includes(this.map_string_to_document_type(x.doc_type))
        );
    }
    ...
    if (territory_types != null && territory_types.length >= 1) {
        search_data.value = search_data.value.filter(x =>
            territory_types.includes(this.map_string_to_territory_type(x.territory))
        );
    }
    ...
    const document_edges = new Array<DocumentEdge>();
   ...
   */
  map_string_to_document_type(value: string): DocumentType | undefined {
   ...
   */
  map_string_to_territory_type(value: string): TerritoryType | undefined {
    switch (value.toUpperCase()) {
      case "UK":
...
format_index_search_results(results: FacetResults<Product>, facet_match: string): Array<SubstanceIndexItem> {
    ...
    return filteredResults.filter(x => x !== undefined) as Array<SubstanceIndexItem>;
}
  ...
  build_filter(
    ...
    let docs_filter =
      documentTypes && this.build_document_types_filter(documentTypes);
    let products_filter =
      productName && this.build_product_name_filter(productName);
    let territories_filter =
      territoryTypes && this.build_territory_types_filter(territoryTypes);

    let filters = [products_filter, docs_filter, territories_filter].filter(
      Boolean
    );
  ...
  build_document_types_filter(documentTypes: DocumentType[]): string | null {
  ...
  build_territory_types_filter(territoryTypes: TerritoryType[]): string | null {
  ...
  build_product_name_filter(productName: string): string {
