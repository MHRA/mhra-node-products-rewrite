...
import {BmgfResult, CollectionCountResults, FacetResults} from "../../types.js";

export class MedicineLevelsInPregnancyDataSource {

    Context: AzureContext;
    // The default instance of an AzureSearchClient will search against the mlip-index
    ...
    async reports_associated_with_substance(substance_name: string) : Promise<SubstanceReports> {
        ...
        const bmgf_result_substance_reports = await this.Context.bmgf_client
                        .build_full_search_with_filter_operator<CollectionCountResults<BmgfResult>>("active_substances",substance_name,"eq","substance");
    ...
    async get_report_count_against_substance_index(letter: string) : Promise<Array<SubstanceIndex>>{
        const facet_search = await this.Context.bmgf_client.build_facet_search<FacetResults<SubstanceIndex>>("facets",letter,"eq");
        //Filter results to only get the the facet results of the query
        return this.format_index_search_results(facet_search, letter);
    }


    async get_pregnancy_reports_search(search: string, first?: Number, skip?: Number, after?: String) : Promise<Reports>{
        const reports = new Reports();
        const report_edges = new Array<ReportEdge>();
        ...
        const search_data = await this.Context.bmgf_client
                        .build_search<CollectionCountResults<BmgfResult>>(query_string);
    ...
    format_index_search_results(results: FacetResults<SubstanceIndex>, letter: string) : Array<SubstanceIndex> {
