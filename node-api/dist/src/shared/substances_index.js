...
import { Product, SubstanceIndex, Documents, DocumentEdge, Substance, Document, } from "../__generated__/resolvers-types.js";
...
export class SubstancesDataSource {
     ...
     */
    async get_substances_index(letter) {
        const keyToCheck = `substance_character_result_${letter}`;
        // Check if memory cache contains letter result
        const cachedResult = memoryCache.get(keyToCheck);
        if (cachedResult !== null) {
            return cachedResult;
        }
        const facet_search = await this.Context.products_client.build_facet_search("facets", letter, "eq");
     ...
     */
    async get_substance_with_products(substance_name) {
        ...
        const collection_search = await this.Context.products_client.build_filter_by_collection_field_search("substance_name", substance_name, "eq", "f");
        const grouped_results = Object.entries(collection_search.value.reduce((groups, item) => {
            ...
            return groups;
        }, {})).map(([product_name, product]) => ({ product_name, product }));
        Object.entries(grouped_results).forEach(entry => {
            ...
            value.product.map(p => {
                console.log("Mapped product is ", p);
