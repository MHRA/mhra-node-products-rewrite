...
export class RedisManager {
     ...
     */
    async get_from_redis(id: string): Promise<JobStatus | null> {
     ...
     */
    async set_in_redis(key: string, value: string): Promise<string | null> {
        const client = this.get_client();
        let result = null;
        try {
            await client.connect();
     ...
     */
    async delete_from_redis(id: string): Promise<number> {
        const client = this.get_client();
        let result = 0;
        try {
            await client.connect();
     ...
     */
    parse_job_status(status: string): JobStatus {
        switch (status) {
            case "Accepted":
