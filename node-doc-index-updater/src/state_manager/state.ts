import { JobStatus, JobStatusResponse } from "../types.js";
import { RedisManager } from "./redis.js"
import * as xmljs from 'xml-js';
import { v4 as uuidv4 } from "uuid";

/**
 * Interface representing the state manager with methods for getting and setting job status.
 */
interface StateManager {
    get_status(id: string): Promise<JobStatusResponse>;
    set_status(id: string, status: JobStatus): Promise<JobStatusResponse>;
}

/**
 * A class that implements the StateManager interface for managing job statuses using Redis.
 */
export class JobStatusClient implements StateManager {

    /**
     * The instance of RedisManager used for interacting with Redis.
     */
    redis_manager: RedisManager | undefined;
     ...
     */
    async set_status(id: string, status?: JobStatus): Promise<JobStatusResponse> {
     ...
     * @returns A Promise that resolves to a JobStatusResponse containing the job id and status.
     */
    async get_status(id: string): Promise<JobStatusResponse> {
        let status = await this.redis_manager?.get_from_redis(id) as JobStatus;
        return new JobStatusResponse(id, status);
    }
     ...
     */
    async get_job_status_json(id: string): Promise<string> {
     ...
     */
    async get_job_status_xml(id: string): Promise<string> {
        const status = await this.get_status(id);
