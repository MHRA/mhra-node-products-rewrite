...
import { CronTimeToWait, Document } from "./types.js";
...
import {SftpClient} from "./storage_client/sentinel.js";
...
const authentication = new AuthManager();
  ...
  const configure_app = (app: express.Express) => {
    app.use(helmet());
    app.use(cors());
  ...
  */
  const register_endpoints = (app: express.Express) => {
  ...
  const register_cron_jobs = async () => {
    const cron_time_to_wait: CronTimeToWait = {
      create_manager_create: process.env.SECONDS_TO_WAIT as unknown as number,
      create_manager_create_clean: process.env.SECONDS_TO_WAIT as unknown as number * 10,
      delete_manager_create: process.env.SECONDS_TO_WAIT as unknown as number,
      delete_manager_create_clean: process.env.SECONDS_TO_WAIT as unknown as number * 10,
    };
 ...
 //Starts our express server after configuring the relevant middleware, endpoints and settings
  const start_server = (app: express.Express, port: number) => {
