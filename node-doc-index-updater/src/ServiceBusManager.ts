...
import {AzureConfig} from "./search.js";
 ...
 */
export abstract class ServiceBusManager {
     ...
     */
    protected constructor(service_bus_queue_name: string){
        ...
        //TODO
        this.azure_service_bus = new AzureServiceBusClient(
            process.env.SERVICE_BUS_CLIENT_NAME,
            service_bus_queue_name,
            "test",
            process.env.SERVICE_BUS_CLIENT_KEY
        );
    }
     ...
     */
    protected async receive_queue_messages<T>() : Promise<Array<T>>{
        ...
        if(message_count > 0){
            ...
            return await this.azure_service_bus.complete_messages<T>(this.queue_name, message_count);
        }
     ...
     */
    protected get_azure_config() : AzureConfig {
        return {
     ...
     */
    public async run_queue_cleanup(): Promise<void> {
