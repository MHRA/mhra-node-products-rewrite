...
import { DocIndexUpdaterQueue } from "../types.js";
...
export class AzureServiceBusClient {
    ...
    constructor(service_bus_namespace?: string,
    ...
    */
    create_client() : ServiceBusClient {
    ...
    new(client: ServiceBusClient) : DocIndexUpdaterQueue {
    ...
    */
    create_sender(queue: string) : ServiceBusSender {
    ...
    */
    create_receiver(queue: string, sub_queue?: "deadLetter" | "transferDeadLetter") : ServiceBusReceiver {
    ...
    */
    async send_message_to_queue(queue: string, message:any, ttl?: number) : Promise<void> {
        try {
            this.client = this.create_client();
    ...
    */
    async complete_messages<T>(queue: string, message_count?: number) : Promise<Array<T>> {
        const processed_messages = new Array<T>();
        await this.client.close();
        this.client = this.create_client();
        ...
        try
        {
            ...
            for (const message of messages) {
                try {
                    const created_document = message.body as T;
    ...
    */
    async check_queue_message_count(queue: string, dlq?: Boolean) : Promise<number> {
    ...
    */
    async stream_messages(queue: string){
