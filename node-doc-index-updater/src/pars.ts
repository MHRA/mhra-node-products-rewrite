import { AzureBlobStorage, StorageFile } from "./storage_client/storage.js";
import { Field } from "formidable";
import {
  BlobMetadata,
  Document,
  DocumentType,
...
export class ParsService {
  ...
  constructor() {
    this.blob_client = new AzureBlobStorage(
  ...
  async queue_pars_upload(
    ...
    const job_ids: string[] = new Array(metadata.length);
    ...
    for (const data of metadata) {
      ...
      let storage_file = await this.add_file_to_temporary_blob_storage(
        job.id,
        file_buffer,
        data.pl_number
      );
      ...
      await this.document_manager.check_in_xml_document(
        document,
        "CREATE",
        "",
        uploader_email
  ...
  //  const job = await this.state_manager.accept_job();
    ...
    await storage_client.delete_blob_by_reference(
      old_blob_reference,
      "include"
    );
   ...
   */
  async add_file_to_temporary_blob_storage(
    job_id: string,
    file_data: Buffer,
    licence_number: string,
    existing_temp_blob?: string
  ): Promise<StorageFile> {
   ...
   */
  async add_file_to_permanent_blob_storage(
    job_id: string,
   ...
   */
  groups_fields_by_product(fields: Field[]): GroupedFields {
   ...
   */
  get_field_as_uppercase_string(fields: Field[], name: string): string | null {
    // Find the field with the specified name and convert its value to uppercase
    return (
      fields.find((field) => field.name === name)?.value.toUpperCase() ?? null
    );
  }
   ...
   */
  product_form_data_to_blob_metadata(
    file_name: string,
    fields: Field[]
  ): BlobMetadata | null {
    // Extract product name from the form data and ensure it exists
    const product_name = this.get_field_as_uppercase_string(
      fields,
      "product_name"
    );
    ...
    const pl_number = this.get_field_as_uppercase_string(
      fields,
      "licence_number"
    );
    if (!pl_number) return null;
    ...
    const territory = fields
      ...
      ?.value.toUpperCase() as TerritoryType | undefined;
   ...
   */
  async read_pars_upload(form_data: any): Promise<[BlobMetadata[], Buffer]> {
    // Extract fields from the form data
    const fields = this.get_fields_from_form_data(form_data);

    // Group fields by product
    const { products, file_name, file_data } =
      this.groups_fields_by_product(fields);

    // Convert product form data to BlobMetadata and filter out null values
    const metadata = products
      .map((fields) =>
        this.product_form_data_to_blob_metadata(file_name, fields)
      )
      .filter((metadata): metadata is BlobMetadata => metadata !== null);
   ...
   */
  get_fields_from_form_data(form_data: any): Field[] {
    const fields: Field[] = [];
