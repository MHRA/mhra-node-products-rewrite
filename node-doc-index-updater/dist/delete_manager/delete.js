...
export class DeleteManager extends ServiceBusManager {
    ...
    constructor() {
        ...
        this.storage_client = new AzureBlobStorage(process.env.STORAGE_CONTAINER_TEMPORARY, '', process.env.STORAGE_ACCOUNT_TEMPORARY, process.env.STORAGE_MASTER_KEY_TEMPORARY);
        this.search_index = new SearchIndex();
        this.azure_service_bus_client = new AzureServiceBusClient(process.env.SERVICE_BUS_NAMESPACE, process.env.DELETE_QUEUE_NAME, process.env.SERVICE_BUS_SHARED_ACCESS_KEY_NAME, process.env.SERVICE_BUS_DELETE_ACCESS_KEY);
    }
    /**
     ...
     */
    async check_in_delete_request(initiator_email, document_id) {
        try {
     ...
     */
    async run_delete_queue() {
        // Receive any messages in the delete queue
        const received_messages = await this.receive_queue_messages();
